...
Download precompiled binarys from the [release page](https://github.com/freeddser/shadowsocks-go/releases). (All compiled with cgo disabled, except the mac version.)

You can also install from source (assume you have go installed):

```
# on server
go get github.com/freeddser/shadowsocks-go/cmd/shadowsocks-server
# on client
go get github.com/freeddser/shadowsocks-go/cmd/shadowsocks-local
```
...
Configuration file is in json format and has the same syntax with [shadowsocks-nodejs](https://github.com/clowwindy/shadowsocks-nodejs/). You can download the sample [`config.json`](https://github.com/freeddser/shadowsocks-go/blob/master/config.json), change the following values:
...
Here's a sample configuration [`client-multi-server.json`](https://github.com/freeddser/shadowsocks-go/blob/master/sample-config/client-multi-server.json). Given `server_password`, client program will ignore `server_port`, `server` and `password` options.
...
Here's a sample configuration [`server-multi-port.json`](https://github.com/freeddser/shadowsocks-go/blob/master/sample-config/server-multi-port.json). Given `port_password`, server program will ignore `server_port` and `password` options.
...
**Use OpenVZ VM that supports vswap**. Otherwise, the OS will incorrectly account much more memory than actually used. shadowsocks-go on OpenVZ VM with vswap takes about 3MB memory after startup. (Refer to [this issue](https://github.com/freeddser/shadowsocks-go/issues/3) for more details.)
